...
* <b>title</b> (default) - Uses the title SVG element for each node and edge <b>g</b> element as generated by [Graphviz](http://www.graphviz.org). For nodes, this is the [<i>node_id</i>](http://www.graphviz.org/content/dot-language) (not to be confused with the node attribute [<i>id</i>](http://www.graphviz.org/content/attrs#did)) and for edges it is the [<i>node_id edgeop node_id</i>](http://www.graphviz.org/content/dot-language). For node and edge sub-elements, the key mode <b> tag-index</b> is used, see below.
...
Set the key mode to *mode*. If the *mode* is not one of the defined key modes above, an error is thrown. Returns the graph renderer instance. The key mode must be set before attaching the [DOT](http://www.graphviz.org/content/dot-language) source string. If it is changed after this, an eror is thrown.
