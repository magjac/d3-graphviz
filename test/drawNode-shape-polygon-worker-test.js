...
describe("drawNode()", () => {
    ...
    tape("Verify that polygon shape is drawn exactly as Graphviz does.", async function (test) {
        ...
        await new Promise(resolve => {
            expectedGraphviz = d3_graphviz.graphviz("#expected-graph", { useSharedWorker: true })
                .on('initEnd', resolve);
        });
        ...
        await new Promise(resolve => {
            expectedGraphviz
                .zoom(false)
                .renderDot('digraph {a [shape="polygon"]}', resolve);
        });
